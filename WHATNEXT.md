...
Phase 12's own evaluation is the best argument for working on this next: a live run against a small local model (`qwen2.5:0.5b`) scored **0% grounding rate** and produced a confirmed hallucinated `"ransomware"` classification with zero supporting evidence (see [docs/evaluation_methodology.md](docs/evaluation_methodology.md)). That's not a bug — it's real, measured evidence that small local models produce fluent-but-ungrounded triage text. Ideas, roughly in order of effort:

- **Re-run the same evaluation against the actual recommended 7–8B model** (`llama3.1:8b-instruct-q4_K_M`) instead of the 0.5B hand-verification model — this project has never actually measured its own recommended default's quality, only a smaller stand-in. Cheap to do, valuable to know.
- **Few-shot examples in the prompts** (`app/triage/prompts.py`) — right now every prompt is zero-shot. One or two worked examples per task (especially `attack_classification`, where the hallucination showed up) would likely improve grounding without any architecture change.
- **A grounding-aware retry**: if `evaluate_grounding()`'s check fails for a given response, retry once with an explicit "your last answer didn't reference any of the incident's actual identifiers — try again" prompt, before falling back to showing the ungrounded result. Currently grounding is only measured, never acted on.
- **A feedback signal from the dashboard itself** — a thumbs up/down on each AI analysis card, persisted, would build a real dataset of "which outputs an analyst actually trusted" over time, and is a natural precursor to any future fine-tuning or few-shot-example curation.
