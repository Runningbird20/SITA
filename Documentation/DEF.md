...
Per `[[recommended-local-model]]` in `TODO.md`'s Architecture Decisions, resolved as a two-tier decision rather than a single pinned model: `Settings.ollama_model` defaults to `qwen2.5:0.5b` — small (~400MB), fast to pull, and light enough to run on modest hardware, chosen specifically so `docker compose up`/`scripts/demo.sh` never forces a multi-gigabyte download or serious hardware just to prove the pipeline works end to end. This is a *quick-start* default, explicitly not a *quality* recommendation — the README's "Choosing an Ollama model" section documents the tradeoff plainly (download size, RAM, CPU-only latency, and output quality, backed by this project's own evaluation results) and recommends a 7–8B instruct model (`llama3.1:8b-instruct-q4_K_M` is what `docker-compose.yml`'s own pull-command comment shows) for anyone using triage output for real, not just confirming the app runs. Both values are fully config-driven (`OLLAMA_MODEL` in `.env`), so switching is a one-line change plus a backend restart, never a code change.
