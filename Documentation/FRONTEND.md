...
Status: complete — and, since Phase 10, relocated rather than replaced (see the third addendum below). This document explains what was built, how the pieces fit together, and why each decision was made — same format as the `PHASE-N.md` reports, but this isn't one of them: it's not a numbered phase from [TODO.md](../TODO.md). It's a cross-cutting meta-tool, requested directly rather than derived from the roadmap. See [README.md](../README.md#loading-synthetic-security-event-data) for the user-facing description and [CLAUDE.md](../CLAUDE.md#repository-map) for the maintenance rule this component comes with.
...
## Addendum: splitting the third tier's color (green → yellow)

The second addendum's `"implemented_static"` tier reused the same green dot as a live `"implemented"`, distinguished only by its label ("Implemented" vs. "Working"). Requested directly: give `"implemented_static"` its own color — yellow/amber — so the two tiers are visually distinct at a glance, not just distinguishable by reading the label text. This matters more as more phases land in the static tier (by Phase 7, five of eight built phases were static-asserted, only three were live-checked), since a wall of green dots was starting to visually claim more runtime verification than actually exists.

Added `--color-yellow`/`--color-yellow-glow`/`--color-yellow-border` alongside the existing green/red/gray palette in `index.css`. `StatusDot.css` and `PhaseRow.css` each split their combined `[data-status="implemented"], [data-status="implemented_static"]` selector into two separate rules — green stays on live `"implemented"`, yellow goes on `"implemented_static"`. `App.tsx`'s summary bar, previously one merged "N implemented" green count, became two counts: "N working" (green) and "N implemented" (yellow), and the footer copy was updated to explain the yellow tier explicitly rather than leaving it implied by the label alone.

Verified: `npm run build`, `npm run lint`, and `npm run format:check` all pass clean. **Not verified**: an actual rendered screenshot, for the same reason as the original build — no browser or screenshot tool available in this environment. The color mapping was confirmed by reading the CSS selectors directly rather than observed visually; flagged explicitly rather than claimed.

## Addendum: relocated to `/status`, not replaced (Phase 10)

The original framing above (and the original title of this document) assumed Phase 10 would **replace** this page — build the real dashboard at `/`, retire the build-status meta-tool. What actually happened, on direct instruction: `react-router-dom` was added, `App.tsx` became a route table, and this page moved unchanged to `/status` while `/` through `/mitre` became the real SOC-style dashboard (see [PHASE-10.md](PHASE-10.md)). The reasoning for keeping it: it still answers a question the real dashboard was never designed to answer — "is each backend phase's surface live right now," independent of whether any actual incident/alert/IOC data exists yet. A reviewer poking at `/status` gets the same honest, evidence-based verification this page always provided; a reviewer at `/` gets the product.

No code in `StatusPage.tsx`, `PhaseRow.tsx`, `StatusDot.tsx`, `phases.ts`, or `useBackendStatus.ts` changed as part of the move beyond the file relocation and updated relative import paths — this remains the same component described above, just reachable at a different URL.

## What this deliberately does not include

No real domain data (events, alerts, incidents, IOCs) on `/status` itself — that page only ever asks "is a phase's backend surface present and healthy," never "show me the data" (the real dashboard, described in [PHASE-10.md](PHASE-10.md), is where the data lives). No historical status tracking (each load/poll is a fresh check; nothing is persisted). No auth (matches the rest of the project pre-Phase-14). As of Phase 9, phases 0, 1, 2, 3, 4, 5, 7, 8, and 9 all have live checks; phases 6 and 10 are static "Implemented" (neither has a REST resource of its own — see DEF.md § Phase 9/10); phases 11–15 have no built surface yet and stay gray, deliberately not guessed at, since guessing a route name that later implementation doesn't match would produce a false negative (or worse, a false positive against an unrelated coincidental route) that's harder to notice than an honest gray dot. When each of those phases is actually built, its `evaluate` function should be added to `phases.ts` as part of that phase's own completion work, per the rule recorded in `CLAUDE.md`.
