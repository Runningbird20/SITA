...
- **`[[dashboard-auth]]` Authentication approach for the dashboard/API — resolved (Phase 14).** Single shared bearer token (`API_AUTH_TOKEN`), not user accounts — proportionate to a local-first, single-operator tool with no multi-tenant concept anywhere in its data model. Disabled by default (empty token) so every existing quick-start command and test keeps working with zero setup; when set, required via `require_auth` on every `/api/v1/*` route, with a real frontend token-entry flow (`AuthGate`). See [DEF.md § Phase 14](Documentation/DEF.md#phase-14-security-hardening).
