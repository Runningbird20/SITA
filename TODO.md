...
### Production deployment with real TLS — resolved

Originally raised in WHATNEXT.md's "Security and deployment hardening" section: `docker-compose.yml`'s `frontend` service never actually used the hardened `production` nginx stage Phase 14 built, and TLS was explicitly out of scope through Phase 14 (plain HTTP, local-only).

**Resolution:** a new, fully self-contained `docker-compose.prod.yml` (deliberately not a partial override of the dev file — avoids depending on Compose's list-merge semantics for something this consequential) builds the `production` frontend target, drops `--reload` from the backend, and adds a `proxy` service (Caddy) as the only container with a published port, terminating TLS and routing same-origin to backend/frontend. `SITA_DOMAIN` (default `localhost`) gets a self-signed certificate with zero setup via `tls internal`; a real domain with live DNS gets a real Let's Encrypt certificate by deleting that one line — documented directly in the new root `Caddyfile`. A real bug caught during verification: the file initially had no `name:` key, so Compose derived the same project name the dev stack uses and bringing it up silently replaced the already-running dev containers rather than running alongside them — fixed with `name: sita-prod`. Verified with a full build-up-curl-down cycle: real HTTPS termination (self-signed cert issued by "Caddy Local Authority"), `/healthz` and `/api/v1/incidents` both correctly proxied, no hardcoded API URL baked into the built JS bundle, and the dev stack confirmed running untouched alongside it under the corrected project name. See [DEF.md § Phase 15](Documentation/DEF.md#phase-15-status-implemented).

### Multi-user / RBAC — resolved

Originally raised in WHATNEXT.md's "Security and deployment hardening" section: the single shared bearer token (`[[dashboard-auth]]`, Phase 14) couldn't answer "nothing tracks *who* triggered a pipeline run or changed an alert's status" by construction — a shared secret has no identity to attribute.

**Resolution:** scoped explicitly as an MVP (confirmed with the user before building — not full enterprise IAM). Named `User` accounts (bcrypt-hashed passwords, two roles: `analyst`/`admin`), session-based `AuthToken`s (hashed, revocable, 7-day default expiry) issued by a real `POST /auth/login`, and an `AuditLogEntry` trail for every mutating action (`pipeline.run`, `feedback.set`, `feedback.clear`), readable via admin-only `GET /audit-log`. Auth is opt-in by DB state, not a config flag — zero `User` rows means the zero-friction default is completely unchanged; creating the first one (`uv run python -m app.auth.cli create-user`, since there's no open self-registration) turns it on globally. `POST /pipeline/run` is now admin-only; a real bug caught by the CLI's own test suite (a `DetachedInstanceError` from reading ORM attributes after the session closed) was fixed before it could have crashed real usage. Frontend `AuthGate` now shows a real username/password form instead of a bare token field, with a `useAuth()` context gating the "Run pipeline" nav button by role. Verified against the real running docker-compose stack (rebuilt for the new `bcrypt` dependency, migration applied to real Postgres, real 401→login→200 and analyst-403/admin-200 role gating over live HTTP) and with a headless-Chromium screenshot pass proving the login form and post-login nav render correctly — verification users deleted afterward to leave the shared dev environment back in its default disabled-auth state. Replaces the single-shared-token model outright rather than keeping both. See [DEF.md § "Multi-user / RBAC (post-roadmap)"](Documentation/DEF.md#multi-user--rbac-post-roadmap).

### Dependency scanning automation — resolved

Originally raised in WHATNEXT.md's "Security and deployment hardening" section: `pip-audit`/`npm audit` run in CI (Phase 14) but nothing automatically opened a PR when a new advisory landed against an already-merged dependency.

**Resolution:** `.github/dependabot.yml` — weekly version-update PRs across `uv` (backend), `npm` (frontend), `docker` (both Dockerfiles' base images), and `github-actions` (the CI workflow's own action pins). Any PR it opens still runs through the existing blocking `pip-audit`/`npm audit` CI checks before merge — this file is the "who opens the PR" half, not a replacement for those checks. Only activates once pushed to GitHub (and needs "Dependabot alerts" enabled in repo settings for a private repo — on by default for public ones); the config's correctness was verified as valid YAML matching Dependabot's schema, not via a live PR, since that requires the file actually living on GitHub's default branch.

### Multi-process metrics/rate limiting — resolved

Originally raised in WHATNEXT.md's "Security and deployment hardening" section: both `app/core/metrics.py` (Prometheus registry) and `app/core/rate_limit.py` were in-process, single-worker state — a documented limitation at the time, but a real gap once `docker-compose.prod.yml` (also post-roadmap) made multiple backend workers an actual deployment shape rather than a hypothetical.

**Resolution:** rate limiting now tries a new `app/core/redis_client.py::get_redis_client()` first (a Redis `INCR`/`EXPIRE` fixed-window counter, genuinely shared across worker processes), falling back to the original in-memory limiter when `REDIS_URL` is unset (unchanged default for native dev and the single-process dev `docker-compose.yml`) — and failing *open* (allowing the request) rather than blocking if Redis is briefly unreachable, since this is an availability safeguard, not an auth boundary. Metrics needed no change to how they're declared — only to how `/metrics` reads them: `PROMETHEUS_MULTIPROC_DIR` (set in `docker-compose.prod.yml`, read directly by `prometheus_client` itself) switches each worker to writing its own counter file instead of pure in-memory state, and the endpoint merges every worker's file via `prometheus_client.multiprocess.MultiProcessCollector` at scrape time. `docker-compose.prod.yml` gained a `redis` service and now runs the backend at `--workers 4` (was 1, specifically because this gap hadn't been closed yet). Verified live against the real stack, not just unit tests: `docker top` confirmed 4 genuine worker processes, 5 real HTTP requests produced an exact Redis counter of `5` (not the under-count an in-memory-per-worker limiter would have produced), and separate per-worker `.db` files were confirmed correctly summed by `/metrics`. See [DEF.md § "Multi-process metrics/rate limiting (post-roadmap)"](Documentation/DEF.md#multi-process-metricsrate-limiting-post-roadmap).

### Statistical/anomaly detection layer — resolved

Originally raised in WHATNEXT.md's "Detection and correlation depth" section: every Phase 3 rule is threshold- or pattern-based, which is explainable but blind to "this is unusual for *this specific* environment" — a fixed threshold either misses a low-and-slow anomaly on a quiet host or never fires on a naturally noisy one.

**Resolution:** a 9th detection rule, `AnomalousEventVolumeRule` (`rule_key="anomalous_event_volume"`), staying fully deterministic per this project's founding principle — the "statistic" is `statistics.mean`/`statistics.pstdev` on integer daily event counts, not a learned model. For every `(source_type, host)` pair, it buckets that host's own prior history into daily counts and flags a day whose count is `z_score_threshold` (default 3.0) standard deviations above that specific host's own baseline mean — adaptive per-host, not a global constant, and skipped entirely when fewer than `min_baseline_days` (default 3) of history exist rather than guessing from too little data. Spans all five source types (a new `DetectionCategory.ANOMALY` value was added, since the existing four categories all describe *what kind* of behavior, not *why it's anomalous*). MITRE-mapped to `T1496` (Resource Hijacking, newly vendored) as the most broadly-fitting technique for an unexplained volume spike, explicitly documented as a reasonable-but-imperfect fit rather than a precise claim. Verified with a new synthetic dataset (`data/synthetic_events/endpoint/anomalous_volume.jsonl`, 6 days on one host, 5 steady baseline days then a 25-event spike) and 5 new unit tests. See [DEF.md § "Post-roadmap addition: Anomalous Event Volume rule"](Documentation/DEF.md#post-roadmap-addition-anomalous-event-volume-rule-9th-detection-rule-a-statisticaladaptive-layer).

### Cross-rule fingerprint dedup — resolved

Originally raised in WHATNEXT.md's "Detection and correlation depth" section: `[[detection-run-idempotency]]`'s fingerprint only prevents the *same* rule from re-alerting on the *same* events; two *different* rules firing on identical matched-event evidence still created two separate `Alert` rows, which correlation would usually merge at the incident level but left the alert list itself noisier than the evidence warranted.

**Resolution:** a new `compute_evidence_fingerprint()` (rule-independent, unlike the existing per-rule `compute_alert_fingerprint()`) lets `run_detection()` recognize when a finding's matched-event set exactly matches an alert that already exists — from this run or an earlier one — and, rather than creating a duplicate `Alert`, records the second rule's key onto the existing alert's `severity_factors["also_detected_by"]`. Made concretely reachable (not just theoretical) by the new `anomalous_event_volume` rule above, which spans every source type and can land on the exact same evidence as a narrower rule like `ssh_brute_force` when an entire day's activity for a host *is* the detected burst — proven with a dedicated synthetic scenario, since no existing real dataset happens to trigger the collision (confirmed empirically: `cross_rule_duplicates_skipped: 0` against every checked-in dataset). No schema change or migration — deliberately scoped below the larger alternative (a first-class many-to-many `Alert`↔`Detection` relationship), which would have meant reworking `Alert.detection_id`, the MITRE rollup, the API schema, and the frontend for a feature whose real-world frequency is currently zero. See [DEF.md § "Post-roadmap addition: cross-rule fingerprint dedup"](Documentation/DEF.md#post-roadmap-addition-cross-rule-fingerprint-dedup).

### Full ATT&CK Enterprise vendoring + candidate-technique retrieval — resolved

Not originally a WHATNEXT.md bullet — raised directly by the user, motivated by Phase 12's own evaluation finding (0% grounding rate, a confirmed hallucinated `"ransomware"` classification) and the [[mitre-data-source]] decision's own framing: Phase 8 deliberately vendored only 6 hand-picked techniques, and the `mitre_suggestion` triage task asked the model to freely recall additional technique_ids from training knowledge against that tiny local dataset — real grounding was structurally impossible, not just a wording problem.

**Resolution:** two parts, requested and confirmed via explicit user choice between build-scope options (full official corpus vs. a larger curated list; a prompt-time retrieval shortlist vs. embeddings vs. no prompt change). `app/mitre/vendor_cli.py` fetches the real, official ATT&CK Enterprise STIX bundle from MITRE's public GitHub repo (a one-time/setup-time script, no runtime network dependency) and rewrote `data/mitre/techniques.json` from 6 curated entries to the full **697** active techniques. `app/mitre/retrieval.py::candidate_techniques_for_incident()` then uses that expanded dataset to build a real, tactically-filtered, keyword-ranked candidate shortlist shown directly in the `mitre_suggestion` prompt (`app/triage/prompts.py`, bumped to prompt version v3), replacing "recall a technique from memory" with "choose from these real, vendored options" — fully deterministic, no embeddings, reusing only the existing SQL-backed `MITRETechnique` table (a considered and rejected alternative: a real embedding/vector-search pipeline, a materially bigger lift with no existing infrastructure to build on). `app/evaluation/ai_grounding.py` gained a complementary `mitre_suggestion_validity_rate` metric — now a meaningful check with ~700 real techniques to validate against, versus 6. Verified: 22 new unit tests across the vendoring parser, the retrieval function, the new prompt shape, and the grounding metric, plus one true integration test proving a real candidate technique_id actually reaches the constructed prompt through the full detection→correlation→triage pipeline. Not verified: an actual before/after grounding-rate measurement against a live model — Ollama wasn't reachable in this environment, stated plainly rather than assumed. See [DEF.md § "Post-roadmap addition: candidate-technique retrieval"](Documentation/DEF.md#post-roadmap-addition-candidate-technique-retrieval).

### Background job execution for triage — resolved

Originally raised in WHATNEXT.md's "Operability and UX polish" section: `POST /api/v1/pipeline/run` ran synchronously — fine against synthetic data (well under a second), but a real `LLMProvider` turns that into a multi-minute blocked request (Phase 15 measured ~5.5 minutes for a 10-incident, 60-call real Ollama run), with the frontend showing nothing but a spinner the whole time.

**Resolution:** scoped via two explicit user choices before building — FastAPI `BackgroundTasks` over a real task queue (RQ/Redis), and per-incident/per-stage progress over a coarse pending/running/done status. Both `POST /pipeline/run` and `POST /pipeline/reanalyze` now return `202 Accepted` with a new `PipelineJob` row immediately, and the actual work (unchanged — same `run_detection`/`run_ioc_extraction`/`run_mitre_mapping`/`run_correlation`/`run_triage` calls as before) runs after the response via `BackgroundTasks`, each opening its own DB session since the request's is already closed by then. A new `GET /pipeline/jobs/{id}` polls status, `current_stage`, and (during triage specifically, via a new `on_progress` callback threaded through `run_triage()`) an incident-count progress counter; `Layout.tsx`'s "Run pipeline"/"Reanalyze" buttons poll every 1.5s and show that progress live instead of a static "Running…". No new dependency — deliberately not RQ, since that would make Redis a hard requirement for a feature that today stays optional (rate limiting already falls back to in-memory without it). A real, non-obvious bug caught by the first test run rather than anticipated: the background job silently wrote to the wrong database in tests, since `BackgroundTasks`-scheduled code bypasses the request-scoped `Depends(get_db)` override entirely — fixed by also monkeypatching the executor's own `SessionLocal`, the same shape already used for CLI tests. Verified: 10 rewritten backend integration tests, 8 new frontend tests (trigger → poll → display, including the failure path), full backend (509 tests) and frontend suites re-run and passing. Not verified: an actual live-Ollama run watched through the browser — not reachable in this environment. See [DEF.md § "Post-roadmap addition: background pipeline jobs"](Documentation/DEF.md#post-roadmap-addition-background-pipeline-jobs).

### Three more detection rules — resolved

Originally raised in WHATNEXT.md's "Detection and correlation depth" section as "natural next candidates" once the DNS tunneling/anomaly-volume rules landed: lateral movement, data exfiltration volume anomalies, privilege escalation patterns.

**Resolution:** `LateralMovementRule` (one account, ≥3 distinct hosts, successful logins, short window — `T1021`), `DataExfiltrationRule` (one internal host, large summed outbound bytes to external destinations — `T1041`), `PrivilegeEscalationRule` (keyword-category command-line matching, same shape as `suspicious_powershell` — `T1548.001`/`.002`/`.003`/`T1136.001`). A real bug caught by running `DataExfiltrationRule` against its own fixture before writing tests, same pattern as the earlier DNS tunneling bug: Python's `ipaddress.is_private` also flags the RFC 5737 documentation ranges this project's synthetic data uses to represent external addresses, so the rule initially found zero external destinations — fixed by checking real RFC 1918 ranges directly. `DetectionRunReport.rules_run` is now 12. 13 new unit tests, 3 new fixtures, verified against the full existing dataset with zero new false positives or cross-rule collisions. See [DEF.md § "Post-roadmap addition: three more detection rules"](Documentation/DEF.md#post-roadmap-addition-three-more-detection-rules-10th12th--lateral-movement-data-exfiltration-privilege-escalation).

### Real ingestion (syslog + file-tail) — resolved

Originally raised in WHATNEXT.md's "Real ingestion, not just synthetic data" section: every event in this project was hand-crafted synthetic data or generator output — no actual log source connector, "cheapest first" named a syslog listener and a file-tail ingester as the first two steps.

**Resolution:** both built, sharing one new sshd-auth-log parser (`app/ingestion/syslog_parser.py`) so RFC 5424 (UDP listener) and classic BSD/RFC 3164 (file tail, the more common real-world default) both reduce to the same recognized message patterns. Both feed the existing `ingest_records()`/`AuthIngestionAdapter` path unchanged — no new ingestion contract, just two new ways a raw-record batch gets produced. No new dependency (stdlib `socketserver` for the UDP listener, polling for the tailer). Verified live, not just unit-tested: a real UDP datagram and a real appended file line both confirmed to produce real `SecurityEvent` rows via a running listener/tailer process. The further-out Kafka/webhook step was skipped — the doc's own reasoning against it (real-deployment-only, meaningfully more infrastructure) still holds. See [DEF.md § "Post-roadmap addition: real ingestion (syslog + file-tail)"](Documentation/DEF.md#post-roadmap-addition-real-ingestion-syslog--file-tail).

### Operability and UX polish (scheduled runs, notifications, export, live updates) — resolved

Originally raised in WHATNEXT.md's "Operability and UX polish" section, all four items together: nothing triggered the pipeline on a cadence, no notification fired on a new critical incident, no way to export an incident, and the dashboard only ever polled rather than receiving live updates.

**Resolution:** `app/scheduler/` (a plain interval loop reusing the existing background-job executor unchanged, so scheduled and manual runs share one job history), `app/notifications/` (a `Notifier` ABC parallel to `LLMProvider`'s own swappable-backend design, a `NoOpNotifier` default, a generic `WebhookNotifier`, wired into `run_correlation()` to fire once per incident reaching CRITICAL severity), `app/export/` (`GET /incidents/{id}/export?format=csv|pdf`, CSV via stdlib, PDF via the one new dependency this pass needed, `fpdf2`), and `app/live_updates/` (`GET /events/stream`, Server-Sent Events, DB-polling rather than a push mechanism). Two real fpdf2 bugs (a Unicode-encoding crash on this project's own generated "→" titles, and a cursor-position bug causing a "not enough horizontal space" crash) and one routing-order bug avoided by reasoning through FastAPI's path-matching order before writing the SSE endpoint (a static `/stream` segment sharing a prefix with an existing `/{event_id}` path-param route would have been shadowed by it) — all caught or avoided during live verification against a real generated incident and a real running server, not just unit tests. Deliberately no new infrastructure (no task queue, no Redis pub/sub) even where a more "correct" design would use one — each subsection reuses a fallback shape this project already established elsewhere (in-memory rate limiting without Redis, `MockProvider`'s zero-network default). Verified: 22 new unit/integration tests, full backend suite (565 tests) re-run and passing. See [DEF.md § "Post-roadmap addition: operability"](Documentation/DEF.md#post-roadmap-addition-operability-scheduled-runs-notifications-export-live-updates).

### Testing and quality bar (frontend coverage, load/stress testing, mutation testing) — resolved

Originally raised in WHATNEXT.md's "Testing and quality bar" section, three items together: frontend test rigor lagging the backend's with no enforced threshold, no load/stress testing beyond a fixed moderate scale, and no mutation testing anywhere.

**Resolution:** frontend coverage (`vite.config.ts`'s `test.coverage`, v8 provider) enforced at a real, earned floor (statements/branches/functions/lines) — deliberately not the backend's 95%, which reflects earned coverage on a much more mature test suite; 24 new tests for the two shared data-fetching hooks, three small shared components, and one representative page, raising the real baseline from ~26% to ~40-48% (a later pass adding the rule-tuning UI's own tests pushed it to ~44-55%, with the floor raised to match each time), with most dashboard pages remaining untested stated as the honest remaining gap rather than implied covered. Load/stress testing reused the existing benchmark harness at 10x scale (15,000 events), finding real ~5.6x correlation throughput degradation exactly where this project's own docs had predicted it without measuring it, plus a genuine 40x-scale (60,000-event) run that didn't complete in 6 minutes; a new concurrent-write-load harness (`app/benchmark/concurrent_load.py`) found SQLite failing 27 of 30 concurrent writes at concurrency 10 while a real (throwaway) Postgres database succeeded 30/30 — a concrete confirmation of this project's existing SQLite-for-solo-dev/Postgres-for-shared-use architecture decision. Mutation testing (`mutmut`, scoped to `correlation/scoring.py` and `detection/base.py` per WHATNEXT's own named scope) found `score_severity()` — the formula every detection rule's severity comes from — had zero direct unit tests, only indirect coverage through categorical rule-test assertions; 19 new precise tests raised its mutation score from 66.9% to 86.3%, with the one remaining `detection/base.py` survivor being a genuine equivalent mutant (unkillable until a future phase gives `asset_sensitivity` a nonzero value) and `correlation/scoring.py`'s 14 survivors left as an honestly-reported, not-yet-chased remaining gap. An incidental real bug fix along the way: two pre-existing frontend test files were failing in this session's environment due to a missing `@testing-library/dom` peer dependency, confirmed present on a clean checkout before this work and fixed as a side effect of installing coverage tooling. A second, later incidental fix: `vite.config.ts`'s coverage config used an `all: true` option that this vitest version (4.1.11) no longer accepts, silently breaking `npm run build` (a type error in `tsc -b`) since the last time this file changed — caught only because this session ran a real production build rather than just the test suite; removed (its intended behavior, showing untested files at 0% rather than omitting them, is vitest's default whenever `include` is set, so no behavior actually changed). See [DEF.md § Phase 11, "Post-roadmap addition: frontend coverage floor"](Documentation/DEF.md#post-roadmap-addition-frontend-coverage-floor) and its adjacent mutation-testing and (§ Phase 12) load/stress-testing sections.

### Analyst feedback closing the loop into rule tuning — resolved

Originally raised in WHATNEXT.md's "Bigger bets" section: marking an alert as a false positive didn't feed back into anything.

**Resolution:** two new mutating endpoints that didn't exist before this — `PUT /alerts/{id}/status` (any authenticated user) and `PATCH /detections/{id}/config` (admin-only, partial merge) — plus a new `DetectionRule.primary_threshold_key` class attribute (declared on 8 of 12 rules) establishing a project-wide convention that raising a rule's primary threshold always means "require more evidence, fewer false positives," regardless of which rule. `app/rule_tuning/analyzer.py::compute_tuning_suggestions()` is fully deterministic — a false-positive-rate computation gated by a minimum sample size, and a fixed-percentage threshold increase — no LLM anywhere in the loop, consistent with the project's founding principle. `GET /detections/tuning-suggestions` surfaces suggestions (registered ahead of `GET /detections/{detection_id}` to avoid the same route-shadowing gotcha already known from `GET /events/stream`); nothing is auto-applied — an analyst reviews and clicks Apply. New frontend: `AlertStatusSelect` (inline status editor on `AlertsPage`) and `TuningSuggestionsPanel` (`DetectionsPage`), both optimistic-update-with-rollback in the same shape as `FeedbackButtons`. Verified live end-to-end against a real (non-fixture) SQLite database with a freshly started server, not just unit/integration tests: ingested real fixture data, ran real detection + correlation, marked a real alert false positive via the live API, confirmed a real audit-log row, confirmed the analyzer correctly suggested a change for the rule with a threshold key and correctly produced nothing for the rule without one, then applied the suggestion via the live config-update endpoint and confirmed it persisted. 7 new analyzer unit tests, 9 new API integration tests, 5 new frontend tests. See [DEF.md § Phase 3, "Post-roadmap addition: rule tuning suggestions from analyst feedback"](Documentation/DEF.md#post-roadmap-addition-rule-tuning-suggestions-from-analyst-feedback).

### A conversational interface with an incident — resolved

Originally raised in WHATNEXT.md's "Bigger bets" section: AI triage was six fixed one-shot tasks per incident, with no way to ask a specific follow-up question.

**Resolution:** a new `ChatMessage` table (not a reuse of `AnalysisResult` — a conversation thread with two authors is a genuinely different shape from six fixed one-row-per-task-per-incident results) records both the analyst's questions and the model's answers. `app/chat/service.py::ask_incident_question()` persists the question immediately, reuses Phase 7's own `build_incident_context`/`render_context_block` for grounding, calls `LLMProvider.generate()` with a new one-field `ChatAnswerOutput` schema (going through the exact same retry/validation pipeline as every other task, not a bypass), and persists the answer (a static fallback message if generation fails, so the thread never shows a blank turn). Deliberately no true token-level streaming and no server-side conversation object — `LLMProvider.generate()` returns one complete response per call, so each turn re-sends the full context plus prior history plus the new question, simpler and more auditable than adding session state to the LLM layer. New endpoints `GET`/`POST /incidents/{id}/chat`, the `POST` rate-limited under the strict tier (a real LLM call, not a cheap read) and audit-logged. New frontend `IncidentChatPanel` on `IncidentDetailPage`, optimistic-update-with-rollback in the same shape as `FeedbackButtons`/`AlertStatusSelect`. A real bug caught by the full test suite during implementation (not by careful reading): an enum edit meant to add `ChatRole` after `PipelineStage` instead spliced in one line early, silently moving `PipelineStage.TRIAGE` into the new `ChatRole` enum — caught immediately by 8 test failures, fixed before it reached any commit. Verified live against a real running server: a real question asked via the live API produced a real persisted `ChatMessage` pair and a real audit-log row, and the strict rate limit was confirmed to actually apply (29 requests succeeded, the 30th returned 429). 13 new backend unit tests, 6 new integration tests, 5 new frontend tests. See [DEF.md § Phase 7, "Post-roadmap addition: a conversational interface with an incident"](Documentation/DEF.md#post-roadmap-addition-a-conversational-interface-with-an-incident).
