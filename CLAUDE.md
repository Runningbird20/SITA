...
- `frontend/` — React + TypeScript + Vite. Until Phase 10 replaces it, the app is a **build status dashboard** (`src/App.tsx`) showing every roadmap phase with a status dot, computed from real evidence — no unconditionally-hardcoded "implemented" flags. The phase manifest lives in `src/data/phases.ts`; each entry's `evaluate` function is one of three kinds, and the type's own doc comment explains when to use which:
  - `liveCheck(...)` — a real HTTP surface exists; status is computed from a live backend response (green "Working", or red "Broken" if the check fails or the backend is unreachable). **Always prefer this** whenever the phase actually has an endpoint to check.
  - `staticImplemented` — the phase is genuinely complete (its own `Documentation/PHASE-N.md` says so, with real tests) but has no live-checkable surface yet (e.g. Phase 3: 7 detection rules, 99 passing tests, but no REST endpoint — deliberately deferred to Phase 9). Green "Implemented", asserted rather than verified at runtime. **Only** for phases that are actually done — never for partial or in-progress work, and never as a substitute for a live check when one is possible.
  - `notImplemented` — nothing built yet — gray. The default for every phase until one of the above applies.

  **When a phase's status changes, update its `evaluate` in `src/data/phases.ts` as part of that phase's own completion work** — checking a `TODO.md` box without also updating this manifest leaves the dashboard lying about what's built, which defeats its purpose.
