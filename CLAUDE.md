...
- `frontend/` — React + TypeScript + Vite. Until Phase 10 replaces it, the app is a **build status dashboard** (`src/App.tsx`) showing every roadmap phase with a live status dot (gray/green/red), computed from real backend checks — no hardcoded "implemented" flags. The phase manifest lives in `src/data/phases.ts`: each entry has an `evaluate` function that either checks something real against `/healthz`/`/openapi.json` (via `src/hooks/useBackendStatus.ts`) or is statically `not_implemented`. **When a phase moves from not-implemented to implemented, add or update its `evaluate` check in `src/data/phases.ts`** — checking a TODO.md box without also updating this manifest leaves the dashboard lying about what's built, which defeats its purpose. Prefer a real live check (a route existing in `/openapi.json`, a health-style signal) over marking a phase `"implemented"` unconditionally.
